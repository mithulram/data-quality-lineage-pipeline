...
def run_pipeline(source_dir: str | Path, output_dir: str | Path) -> PipelineResult:
    ...
    _write_summary(output / "run_summary.json", result, issues)
    _write_lineage(output / "lineage_manifest.json", source, output)
...
def _validate_order(
    order: dict[str, str], customer_ids: set[str], seen_order_ids: set[str]
) -> list[QualityIssue]:
    issues: list[QualityIssue] = []
    ...
    if not order_id:
        ...
        issues.append(QualityIssue(order_id, customer_id, "order_id_unique", "Order ID already appeared in the source."))
    if customer_id not in customer_ids:
        issues.append(QualityIssue(order_id, customer_id, "customer_reference_exists", "Customer ID is absent from customers.csv."))
    try:
        amount = Decimal(order["amount_eur"])
        if amount <= 0:
    ...
    try:
        datetime.fromisoformat(order["order_timestamp"])
    except ValueError:
...
def _write_summary(path: Path, result: PipelineResult, issues: list[QualityIssue]) -> None:
...
def _write_lineage(path: Path, source: Path, output: Path) -> None:
    payload = {
        "pipeline": "data-quality-lineage-pipeline",
        "datasets": [
            {"name": "raw_orders", "path": str(source / "orders.csv"), "role": "source"},
            {"name": "raw_customers", "path": str(source / "customers.csv"), "role": "source"},
            {"name": "quarantined_orders", "path": str(output / "quarantined_orders.csv"), "role": "quality-output"},
            {"name": "curated_orders", "path": str(output / "curated_orders.parquet"), "role": "analytical-output"},
        ],
        "transformations": [
            {
                ...
                "rules": ["order_id_required", "order_id_unique", "customer_reference_exists", "amount_positive", "timestamp_iso8601"],
            },
            {
                "name": "enrich_orders",
